...
import { BookOpen, Search, Menu, LayoutDashboard, X, Home, Library, Bookmark, Book, User } from 'lucide-react';
...
export default function Navbar() {
  ...
  const { subjects, bookmarkedCourses, courses, teachers } = useAppData();
  ...
  const searchResults = () => {
    ...
    return {
      courses: courses.filter(c => c.title.toLowerCase().includes(query) || c.description.toLowerCase().includes(query)).slice(0, 3),
      teachers: teachers.filter(t => t.name.toLowerCase().includes(query) || t.subject.toLowerCase().includes(query)).slice(0, 3),
      ...
      <div className="mx-auto px-4 md:px-8 lg:px-12 h-16 flex items-center justify-between max-w-[1600px]">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm font-medium">
          ...
          {subjects.map(subject => (
            <Link key={subject.id} to={`/subject/${subject.id}`} className="text-gray-300 hover:text-[#00a2ff] transition-colors">{subject.name}</Link>
          ))}
        </div>

        <div className="flex items-center gap-2 md:gap-4">
          {bookmarkedCourses.length > 0 && (
            <Link to="/saved" className="hidden md:flex relative p-2 text-gray-300 hover:text-cyan-400 transition-colors">
      ...
      <div className="p-6 flex flex-col h-full">
        ...
        <div className="flex flex-col gap-2 flex-grow overflow-y-auto pb-4">
          <div className="md:hidden flex flex-col gap-2 mb-4 border-b border-white/10 pb-4">
             <Link to="/admin" onClick={() => setIsOpen(false)} className="flex items-center gap-3 p-3 rounded-xl text-gray-300 bg-white/5 hover:bg-white/10 hover:text-white transition-colors border border-white/5">
                <User className="w-5 h-5 text-purple-400" /> Admin/Editor Login
             </Link>
          </div>
          
          <div className="mt-2 mb-2 px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Subjects</div>
          {subjects.map((subject, idx) => {
            const colors = ["bg-blue-400", "bg-green-400", "bg-purple-400", "bg-pink-400", "bg-yellow-400"];
            const dotColor = colors[idx % colors.length];
            return (
              <Link key={subject.id} to={`/subject/${subject.id}`} onClick={() => setIsOpen(false)} className="flex items-center gap-3 p-3 rounded-xl text-gray-300 hover:bg-white/5 transition-colors">
                <div className={`w-2 h-2 rounded-full ${dotColor}`}></div> {subject.name}
              </Link>
            )
          })}
        </div>
